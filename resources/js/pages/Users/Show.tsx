...
export default function UserShow({ user }: Props) {
  ...
  const updateKycStatus = (status: "pending" | "approved" | "rejected") => {
    router.post(`/users/update-kyc/${user.id}`, { kyc_status: status });
  };
      ...
      <div className="space-y-6 p-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-3">
            <Button variant="outline" asChild className="border-2 dark:border-gray-600">
              <Link href="/users">
                ← Back to Users
              </Link>
            </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border-0 shadow-lg">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-blue-600 dark:text-blue-400">Account Status</p>
                  <div className="mt-2">
                    {getStatusBadge(user.is_active == 1 ? "active" : "inactive")}
                  </div>
                </div>
              ...
              <Button
                ...
                variant={user.is_active == 1 ? "destructive" : "default"}
                size="sm"
          ...
          <Card className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 border-0 shadow-lg">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-green-600 dark:text-green-400">KYC Status</p>
                  <div className="mt-2">
          ...
          <Card className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border-0 shadow-lg">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-purple-600 dark:text-purple-400">Member Since</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white font-mono font-medium">#{user.id.toString().padStart(6, '0')}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600 dark:text-gray-400">KYC Documents</label>
                  <p className="text-gray-900 dark:text-white font-medium">{user.kyc_documents.length} documents uploaded</p>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="border-0 shadow-lg dark:border dark:border-gray-700 dark:bg-gray-800/50">
          <CardHeader className="bg-gray-50 dark:bg-gray-700/50 border-b dark:border-gray-600">
            <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
              <span className="text-xl">📄</span>
              KYC Documents ({user.kyc_documents.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            {user.kyc_documents.length > 0 ? (
              <div className="space-y-4">
                {user.kyc_documents.map((doc) => (
                  <div key={doc.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow dark:border-gray-600 dark:bg-gray-700/30 dark:hover:bg-gray-700/50">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-400">{doc.original_filename}</p>
                        </div>
                      </div>
                    ...
                    {doc.rejection_reason && (
                      <div className="mt-3 p-3 bg-red-50 dark:bg-red-900/20 rounded border border-red-200 dark:border-red-800">
                        ...
                        <p className="text-sm text-red-600 dark:text-red-400">{doc.rejection_reason}</p>
                      </div>
                    )}
              ...
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No KYC Documents</h3>
                <p className="text-gray-600 dark:text-gray-400">This user hasn't uploaded any KYC documents yet.</p>
              </div>
            )}
          </CardContent>
        </Card>
